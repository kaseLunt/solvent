...
- Create: `internal/decode/configurator.go` + `configurator_test.go` (typed events for every topic0 the Task-1 sweep ruled param-bearing: `CollateralConfigurationChanged`, `EModeCategoryAdded`/`EModeAssetCategoryChanged` (names per sweep), liquidation-fee/cap events; strict per-topic0 decode under the `checkCanonicalData` canons).
