/**
 * An instant in prose: `2026-08-08T20:00:00Z` reads `Aug 8, 20:00 UTC`.
 *
 * Built from the ISO string's OWN UTC fields — the text is parsed, never handed to a `Date`: no browser zone, no
 * browser clock, no locale formatter can move the hour or the day a reader is told. Seconds are dropped. The exact
 * wire string stays verbatim wherever a value is stated exactly (a chip, a tile sub, a table cell, the drawer).
 *
 * The year prints (`Aug 8, 2025, 20:00 UTC`) exactly when it is not the year of `referenceIso` — the `served_at` of
 * the envelope that carried the instant. With no reference, or a reference that is not itself a well-formed UTC
 * instant, the year always prints: an instant is never left to a year the caller could not name.
 *
...
function utcFields(iso: string): UtcFields | null {
  ...
  const [, year = "", mo = "", d = "", hour = "", minute = "", second = "00"] = match;
