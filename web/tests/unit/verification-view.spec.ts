...
test("a withheld Cash engine's census is a refusal, never its card's placeholder count: the compute step prints the batch and names the census withheld, in the refused tone — never '0 Cash accounts', never neutral", () => {
  ...
  expect(compute.sentence).toBe(`Batch 1, computed ${nb("Jul 29, 10:00 UTC")}; the Cash book is withheld this batch (collateral-flag custody unproven).`);
  ...
  expect(missing.sentence).toBe(`Batch 1, computed ${nb("Jul 29, 10:00 UTC")}; the Cash engine is missing from this batch.`);
...
test("the four steps carry the Overview's numbers: the OP block, the batch, the gated tally, the endpoint count", () => {
  ...
  expect(steps.map((s) => s.ordinal)).toEqual(["01", "02", "03", "04"]);
  ...
  expect(compute.value).toBe("1");
  expect(compute.sub).toBe("batch · 2 Cash accounts");
  // The batch's instant is the reader's: the exact wire instant is the Book's record, not this sentence's.
  expect(compute.sentence).toBe(`Batch 1, computed ${nb("Jul 29, 10:00 UTC")}: every position's health from exact integers, never floats.`);
...
test("a count is a real plural: one provenance row, fifteen provenance rows; one probe record, one note", () => {
  ...
  expect(probesSummary(EVIDENCE_MANIFEST)).toBe("1 probe record · 1 manifest note");
  const emptied = structuredClone(EVIDENCE_MANIFEST);
  emptied.probe_records = [];
  emptied.notes = [];
  expect(probesSummary(emptied)).toBe("0 probe records");
});
...
test("a step is named once, by the shared vocabulary: the two pages that draw the pipeline cannot count or name its steps differently", () => {
  const steps = pipelineSteps(META, EVIDENCE_MANIFEST, read(BOOK));
  expect(steps.map((step) => [step.ordinal, step.label])).toEqual(steps.map((step) => [PIPELINE_STEPS[step.key].ordinal, PIPELINE_STEPS[step.key].name]));
  expect(steps.map((step) => `${step.ordinal} · ${step.label}`)).toEqual(["01 · Index", "02 · Compute", "03 · Verify", "04 · Serve"]);
  ...
  // A step with no figure keeps its name: the name is the step's, not its reading's.
  expect(pipelineSteps(null, null, UNREAD).map((step) => step.label)).toEqual(["Index", "Compute", "Verify", "Serve"]);
});
...
test("a receipt that failed turns the Verify step crit", () => {
  expect(byKey(pipelineSteps(META, EVIDENCE_PROOF_FAILED, read(BOOK)), "verify")).toMatchObject({ value: "84/87", sub: "checked rows exact · 3 drifted", tone: "crit" });
});
...
test("a drifted row is counted, never 'named': the manifest carries the receipt's tallies and no row, so the Verify step says how many drifted and the drawer says where the rows are recorded", () => {
  ...
  expect(byKey(failed.steps, "verify").sentence).toBe("84 of 87 checked rows matched the chain exactly; 3 rows drifted. The manifest carries the tallies, not the rows.");
  // A path is the drawer's, never a step's sentence.
  ...
  const oneView = view(ok(one));
  expect(byKey(oneView.steps, "verify").sentence).toBe("86 of 87 checked rows matched the chain exactly; 1 row drifted. The manifest carries the tallies, not the rows.");
  expect(oneView.doctrine).toContain("The drifted rows are recorded in the committed drift report.");
  expect(JSON.stringify(oneView)).not.toContain("db-host");
...
test("a receipt that gated NO rows proves nothing: never 'All 0 checked rows matched' — the finding is withheld in the refused register on the headline, the chip, the step, the card and the drawer", () => {
  ...
  expect(proof.rows[0]).toEqual({ label: "Status", value: "Nothing proven · the run checked no rows", tone: "refused" });
...
test("a drifted receipt: warn tone, the warn receipt chip, the drift counted on the receipt line — a weld short names the weld", () => {
  ...
  expect([card.rule, card.status.tone, card.rows[0]?.tone, card.rows[1]?.tone]).toEqual(["warn", "warn", "warn", "warn"]);
  expect(proofSubjectEvidence(drifted).sections[1]?.rows.find((r) => r.label === "Checked rows")?.tone).toBe("warn");
  // A tally short with nothing drifted is its own fault, counted once — never repeated as the detail beside it.
  const tallyShort = structuredClone(EVIDENCE_MANIFEST);
...
test("no committed receipt: receipt none, the refused register, the proof pin and receipt chips refused, the absence named as an absence — the served reason in the dek", () => {
...
test("no servable batch: the proof's finding keeps the receipt's tone and the absence is said in ink beside it; the live batch and key chips refuse, the key is never fabricated, the proof stands", () => {
  ...
  expect(v.chips[3]).toMatchObject({ value: "none", tone: "refused", title: "no batch, no key; never fabricated" });
...
test("a wire that claims no_batch beside a non-null substrate is demoted everywhere: headline, chips, doctrine, card", () => {
  ...
  expect(v.headline.tone).toBe("ok");
  expect(v.chips[1]).toMatchObject({ value: "none", tone: "refused" });
  expect(v.chips[3]).toMatchObject({ value: "none", tone: "refused" });
  expect(JSON.stringify(v)).not.toContain(REAL_KEY);
  expect(v.doctrine.at(-1)).toContain("Batch — (no servable batch, and nothing fabricated)");
  const cards = subjectCards(doctored);
  expect(cards.live.status).toEqual({ text: "No servable batch", tone: "refused" });
  expect(JSON.stringify(cards)).not.toContain(REAL_KEY);
});

test("evidence unavailable: its own register, never a refusal — the reader's cause in the dek with the status when the service answered, the service's words on the state card, nothing substituted, the Verify step unread", () => {
  const timed = view({ phase: "error", message: "503 no_batch: no complete risk batch is available (http://api/v1/evidence)", retryAfterSeconds: 30, status: 503 });
  expect(timed.state).toBe("unavailable");
  expect(timed.receipt).toBe("unavailable");
  expect(RECEIPT_TONE[timed.receipt]).toBe("unavailable");
  expect(timed.headline.tone).toBe("absent");
  ...
  expect(timed.doctrine.at(-1)).toBe("Nothing is substituted for it: no cached proof, no assumed batch, no fabricated key.");
...
test("loading: the headline in its own words, every chip pending and none refused, nothing claimed", () => {
  ...
  expect(v.headline).toEqual({ emphasis: "Loading this deployment's verification record…", rest: "", tone: "absent", dek: VERIFICATION_LOADING_DEK });
...
test("nothing is green under a receipt of no rows: a weld of 0/0 exact proves nothing, so no pill, row or fold row of the proof card — and no row of its drawer — wears the ok tone", () => {
  ...
  expect(proof.rows.map((r) => [r.label, r.value, r.tone])).toEqual([
    ["Status", "Nothing proven · the run checked no rows", "refused"],
...
test("the proof card: the pin pill, the answer rows, the hazards hoisted, fifteen provenance rows in three sections", () => {
  ...
  expect(proof.explain).toBe("Explain the proof subject");
...
test("the proof card's failing and absent arms: the rejected pill with its detail, the short weld crit; the absent receipt's pill and reason", () => {
  ...
  // An absence the wire stated is the refused register, never the unavailable one a failed fetch wears, and never crit.
  expect(absent.status).toEqual({ text: "No committed receipt", tone: "refused" });
  expect(absent.rule).toBe("refused");
  expect(absent.rows[0]).toEqual({ label: "Status", value: "No committed receipt · no committed receipt artifact is present in this deployment", tone: "refused" });
...
test("the live card: the pill, the takeaway, the key row with its copy name, the digest and note folded and counted", () => {
  ...
  // Serving is posture, never health: the live accent, never the pass colour.
  expect(live.status).toEqual({ text: "Serving · watermarked", tone: "live" });
  expect(live.rule).toBe("live");
  expect(live.explain).toBe("Explain the live subject");
...
test("the live card's hazards and absence: a digest gap is an answer row and the count follows; no batch fabricates no key", () => {
  ...
  // No servable batch is an absence the wire stated: the refused register on the pill, the reason in ink, the card's rule still live.
  expect(absent.status).toEqual({ text: "No servable batch", tone: "refused" });
  expect(absent.rule).toBe("live");
  expect(absent.takeaway).toBeNull();
  expect(absent.fold).toBeNull();
  expect(absent.rows[0]).toEqual({ label: "Reason", value: EVIDENCE_NO_BATCH.substrate_unavailable_reason ?? "∅", tone: "default" });
...
test("probe rows keep the card's columns and words: path and note, refusals dimmed, the counted summary, the empty statement", () => {
  ...
  expect(probesSummary(leaking)).toBe("1 probe record");
...
// ---- one verdict, one tone ------------------------------------------------------------------------------------------

test("one map for the receipt's tone: exact ok, drift warn, failed crit, no rows or no receipt refused, a fetch that failed its own unavailable register — never the refused one — and a read in flight pending", () => {
  expect(RECEIPT_TONE).toEqual({ exact: "ok", drift: "warn", failed: "crit", empty: "refused", none: "refused", unavailable: "unavailable", pending: "pending" });
  // A failed receipt is crit everywhere the receipt is spoken: the finding, the Verify step, the chip, the card's rule.
  const failed = view(ok(EVIDENCE_PROOF_FAILED));
  expect(failed.headline.tone).toBe("crit");
  ...
  // No committed receipt: the refused register, dashed — an absence the wire stated.
  expect(subjectCards(EVIDENCE_NO_RECEIPT).proof.rule).toBe("refused");
  expect(subjectCards(EVIDENCE_NO_RECEIPT).proof.status).toEqual({ text: "No committed receipt", tone: "refused" });
  // The live subject is posture, not health: its rule and its pill are the live accent, never green.
  expect(subjectCards(EVIDENCE_MANIFEST).live.rule).toBe("live");
  expect(subjectCards(EVIDENCE_MANIFEST).live.status).toEqual({ text: "Serving · watermarked", tone: "live" });
  expect(subjectCards(EVIDENCE_NO_BATCH).live.status).toEqual({ text: "No servable batch", tone: "refused" });
});

test("a fetch that failed is never a refusal: the absent headline, a dek that says only what failed — step 02 still shows the batch the book served — the unavailable chips, the Verify step's unavailable state, and the service's words disclosed in the page's card", () => {
  const verbatim = "503 no_batch: no complete risk batch is available (http://api/v1/evidence)";
  const v = view({ phase: "error", message: verbatim, retryAfterSeconds: null, status: 503 });
  expect(v.state).toBe("unavailable");
  expect(v.receipt).toBe("unavailable");
  expect(v.headline).toEqual({
    emphasis: "The verification record could not be fetched.",
    rest: "",
    tone: "absent",
    dek: "The service did not answer the proof request (HTTP 503), so there is no proof to show.",
  });
  // The book answered: the Live batch chip names its batch; every other chip is unavailable, in words, never refused.
  expect(v.chips).toEqual([
    { label: "Proof pin", value: "unavailable" },
    { label: "Live batch", value: "1" },
...
test("the Verify step states its register with no figure: pending in flight, unavailable after a failed read, refused where the wire stated no receipt — and the compute step's instant is the reader's", () => {
  expect(byKey(pipelineSteps(null, null, UNREAD, { meta: true, evidence: true }), "verify").state).toBe("pending");
  expect(byKey(pipelineSteps(null, null, UNREAD), "verify").state).toBe("unavailable");
  expect(byKey(pipelineSteps(META, EVIDENCE_NO_RECEIPT, read(BOOK)), "verify").state).toBe("refused");
  expect(byKey(pipelineSteps(META, EVIDENCE_MANIFEST, read(BOOK)), "verify").state).toBeUndefined();
  expect(byKey(pipelineSteps(null, null, FAILED), "compute").state).toBe("unavailable");
  expect(byKey(pipelineSteps(META, EVIDENCE_MANIFEST, NO_BATCH), "compute").state).toBe("refused");
  expect(byKey(pipelineSteps(META, EVIDENCE_MANIFEST, read(BOOK)), "compute").sentence).toBe(
    `Batch 1, computed ${nb("Jul 29, 10:00 UTC")}: every position's health from exact integers, never floats.`,
  );
});
