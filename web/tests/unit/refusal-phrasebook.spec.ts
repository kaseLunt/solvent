...
test("known codes lead with a plain cause; unknown codes fall back to the detail, then the code", () => {
  expect(plainCause("SWEEP_NEVER")).toBe("collateral never read");
  ...
  expect(plainCause("SWEEP_NEVER", "ignored — the phrasebook wins for known codes")).toBe("collateral never read");
});
