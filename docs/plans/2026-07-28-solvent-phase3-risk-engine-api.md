...
> discriminating vectors); P-2 = the drafted two-step HF model FALSIFIED — **P-2's fused-floor
> conclusion was itself SUPERSEDED 2026-07-29 (rev-3): the deployed law is the wadDiv half-up
> composite over ceil-summed debt (see the P-2 supersession banner in p3-probes.md and
> p3-consults/risk-quant-component4-7-ruling.md); the two-step refutation stands**; eMode
> settled at 0 both ways; BTC leg IN (4.11%); backtest
...
- Create: `internal/decode/configurator.go` + `configurator_test.go` — typed events for the COMPLETE 20-topic0 inventory in `recon/p3-probes.md` (zero UNKNOWNs; configurator proxy `0x8438F4D29D895d75C86BDC25360c25eF0607E65d`, stream startBlock 20,625,519), strict per-topic0 decode under the `checkCanonicalData` canons, refuse-loud on any topic0 outside the inventory. **A1 (binding, AS CORRECTED by Wave 2a's four-witness proof): `ReserveInitialized` has TWO indexed args (`topics[1]`=asset, `topics[2]`=aToken) and a strict 96-byte body of (stableDebtToken, variableDebtToken, interestRateStrategy) — decoded by canonical-topic0 registration + hand-authored strict reader; the probe's original field-order reading would have put the stable-debt token in the aToken column.** Backfill posture per probes A2/A3 as CORRECTED by the pre-dispatch consult (`.superpowers/sdd/p3-consults/chain-truth-task2-brief.md` R6.4 — the walker has NO adaptive halving; that phrase was posture, not mechanism): **window 2000 on the existing daemon endpoint config** (the exact posture the ETH streams backfilled through; ≈2.5k getLogs one-time, jagged under dRPC A2 flapping but loud and self-healing); Alchemy window-10 is an OPERATOR playbook for targeted ranges, never automatic, never bulk on the shared free-tier key (it 429-starves the live daemon; observed twice). **The consult archive is NORMATIVE for this task's wave briefs** — its 6-item blocking list (refuse-loud in the ParamRunner; ApplyParamEvents full two-arm gate block; RewindParams consumes `rewindTarget`; three-leg harness; canonical-topic0 + strict 3-word ReserveInitialized reader; window 2000) gates acceptance alongside this plan.
...
**Scope — THREE LEGS (the pre-probe one-range sketch was REFUTED by chain geometry: first liquidation is 313k blocks after the only config event, and the Aave engine's genesis invariant forbids mid-history derived welds; full structure in `.superpowers/sdd/p3-consults/chain-truth-task2-brief.md` R5, NORMATIVE):** (1) genesis-cluster leg — fork ~20,714,020 hash-pinned, walk 20,713,910→20,714,007 (window 10, `ANVIL_FORK_RPC_ETH` on the Alchemy key — the sanctioned targeted carve-out), assert raw bytes == fork getLogs == the committed fixture bytes (independent second witness), derived positions vs fork views, param rows vs `getConfiguration@fork`; pins frozen only AFTER Task 2's live backfill lands; (2) liquidation leg — 21,469,973–21,469,982, raw bytes + strict decode ONLY (no position weld; never start the Aave engine mid-history); (3) **the reorg leg**: `anvil` snapshot → divergent re-mine → walker `Rewind` → durable epoch → `RewindDerived` + `RewindParams` → a riskd-shaped consumer refuses at the gate (`acked_epoch < max_epoch`) → post-ack recompute matches the re-mined chain, **including proof that a param row changed by the re-mine is REPLACED, not orphaned** (Codex round 1 [H3]). The riskd gate-refusal assertion targets the Task 5 watermark reader; until Task 5 lands, leg (c) asserts through a thin test consumer using `store.DeriveCursorStates`/`MaxReorgEpochs` directly (same predicate, promoted to the real reader in Task 5's wave).
