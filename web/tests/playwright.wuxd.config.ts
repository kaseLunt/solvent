// W-UX-D-scoped Playwright config: identical projects to
// ../playwright.config.ts, but on its OWN port (3812) with
// reuseExistingServer OFF — the w2/w4/w5/w6/wuxb wave convention:
//
//   npm run build && npx playwright test -c tests/playwright.wuxd.config.ts
//
...
// working tree causes two PRE-EXISTING failures there). That drift is not
// this wave's to fix or to gate on; the integrator's default-config run
// still executes it.
