  ...
  withResult("dm_rate_horizon_plus_200bps", (r) =>
    !r.projection
      ...
      : {
          ...
          projection: {
            ...r.projection,
            horizons: r.projection.horizons.map((h, i) =>
              i === index ? { ...h, becomes_liquidatable: becomes } : h,
            ),
          },
        },
  );
const PROJECTION_LABEL = "Debt Manager borrow APY +200bps (PROJECTION)";
const PROJECTION_DEK =
  "Room today $190.50; 30d: +$7.92 interest; 90d: +$23.77 interest.";
...
test("idle and invalid: no address is a prompt, a bad address is a refusal; nothing is looked up", () => {
  ...
  expect(idle.headline).toEqual({
    emphasis: "Stress one address.",
    rest: "",
    tone: "refused",
    dek: "Enter an address; the committed scenarios are applied to its Cash position.",
  });
  expect(idle.rows).toEqual([]);
  expect(idle.tiles).toBeNull();
  const invalid = addressWorkspace({
    address: "0xnope",
    view: view({ address: "0xnope", valid: false }),
    selectedId: null,
  });
  expect(invalid.state).toBe("invalid");
  expect(invalid.headline.emphasis).toBe("Not an address.");
  expect(invalid.headline.dek).toBe(
...
test("loading and unavailable follow the lookup, then the stress lookup", () => {
  ...
  const stressLoading = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: view({
      lookup: { phase: "ready", value: lookup(DEMO_ADDRESS_NEAR) },
    }),
  ...
  expect(stressLoading.headline.emphasis).toBe(
    "Running the committed scenarios for 0x7a3f…c21e…",
  );
  const u = addressWorkspace({
    ...
    view: view({
      lookup: {
        phase: "error",
        message: "rate limited (429), retry after 30s",
      },
    }),
    selectedId: null,
  });
  expect(u.state).toBe("unavailable");
  expect(u.headline.emphasis).toBe(
    "The lookup for 0x7a3f…c21e could not be completed.",
  );
  expect(u.headline.dek).toBe(
    "Rate limited (429), retry after 30s. Nothing about this address is known from a failed lookup.",
  );
  const su = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: view({
      lookup: { phase: "ready", value: lookup(DEMO_ADDRESS_NEAR) },
      stress: {
  ...
  expect(su.headline.emphasis).toBe(
...
test("no position and withheld are the stress reading's own words", () => {
  const none = addressWorkspace({
    ...
    view: view({
      address: NOT_FOUND_ADDR,
      lookup: { phase: "ready", value: lookup(ADDRESS_NOT_FOUND) },
      stress: {
        phase: "ready",
        value: lookup({
          ...DEMO_STRESS_NEAR,
  ...
  expect(none.headline.emphasis).toBe(
    `No Cash position for ${NOT_FOUND_ADDR.slice(0, 6)}…${NOT_FOUND_ADDR.slice(-4)} in batch 1.`,
  );
  ...
  expect(withheld.headline.emphasis).toBe(
    "Cannot say — the Cash book is withheld for 0x7a3f…c21e.",
  );
...
test("rows: the demo near account under its three scenarios, the selection, the before/after tiles in the Inspector's registers", () => {
  ...
  expect(w.rows.map((r) => r.id)).toEqual(
    DEMO_STRESS_NEAR.scenarios.map((s) => s.id),
  );
  expect(w.selected?.id).toBe("eth_minus_30");
  expect(w.batchId).toBe(18251);
  expect(w.decimals).toBe(6);
  expect(w.headline.emphasis).toBe(
    "0x7a3f…c21e becomes liquidatable under ETH -30 percent.",
  );
  expect(w.headline.tone).toBe("crit");
  expect(w.headline.dek).toBe(
  ...
  const proj = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: near(),
    selectedId: "dm_rate_horizon_plus_200bps",
  });
  expect(proj.selected?.projection).not.toBeNull();
  expect(proj.headline).toEqual({
    emphasis: `0x7a3f…c21e stays inside its cap through 90d under ${PROJECTION_LABEL}.`,
    rest: "",
    tone: "ok",
    dek: PROJECTION_DEK,
  });
...
test("a projection's horizons decide: a liquidatable horizon is named in the warn tone, an unknowable one is a refusal naming it", () => {
  const within = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: nearWith(projected(1, true)),
    selectedId: "dm_rate_horizon_plus_200bps",
  });
  expect(within.headline).toEqual({
    emphasis: `0x7a3f…c21e becomes liquidatable within 90d under ${PROJECTION_LABEL}.`,
    rest: "",
    tone: "warn",
    dek: PROJECTION_DEK,
  });
  const unknown = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: nearWith(projected(0, null)),
    selectedId: "dm_rate_horizon_plus_200bps",
  });
...
test("a refused Cash position prints no before figure; a negative wire figure prints nothing on its side", () => {
  const r = addressWorkspace({
    ...
    view: view({
      address: DEMO_REFUSED_ADDR,
      lookup: { phase: "ready", value: lookup(DEMO_ADDRESS_REFUSED) },
      stress: { phase: "ready", value: lookup(DEMO_STRESS_NEAR) },
    }),
    selectedId: null,
  });
  ...
  const t = r.tiles;
  if (t === null)
    throw new Error("a refused position beside rows still has tiles");
  ...
  expect(r.headline).toEqual({
    emphasis: "ETH -30 percent does not apply to 0x4444…4404.",
    rest: "",
    tone: "refused",
    dek: "Not evaluated for this account.",
  });
  const negative = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: nearWith(
      withResult("eth_minus_30", (x) =>
        !x.after ? x : { ...x, after: { ...x.after, debt_usd: "-4822000000" } },
      ),
    ),
    selectedId: "eth_minus_30",
  });
  expect(negative.tiles?.debtBefore).toEqual({
    value: "$4,822",
    tone: "neutral",
  });
  ...
  expect(negative.headline).toEqual({
    emphasis:
      "Cannot say whether 0x7a3f…c21e becomes liquidatable under ETH -30 percent.",
    rest: "",
    tone: "refused",
...
test("an unknowable after verdict refuses its figures beside Not computed, and the headline is a cannot-say", () => {
  const u = addressWorkspace({
    ...
    view: nearWith(
      withResult("eth_minus_30", (x) =>
        !x.after ? x : { ...x, after: { ...x.after, liquidatable: null } },
      ),
    ),
    selectedId: "eth_minus_30",
  });
...
test("a projection over an uncomputable spot yields no verdict word: the horizons are not consulted", () => {
  const spot = (patch: Partial<NonNullable<StressResult["after"]>>) =>
    nearWith(
      withResult("dm_rate_horizon_plus_200bps", (x) =>
        !x.after ? x : { ...x, after: { ...x.after, ...patch } },
      ),
    );
  const negative = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: spot({ debt_usd: "-4822000000" }),
    selectedId: "dm_rate_horizon_plus_200bps",
  });
  ...
  expect(`${negative.headline.dek} ${unknown.headline.dek}`).not.toMatch(
    /shock/,
  );
...
test("a projection beside a missing side yields no verdict word: a side the tiles refuse refuses the headline, missing as much as unreadable or unknowable", () => {
  const noAfter = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: nearWith(
      withResult("dm_rate_horizon_plus_200bps", (x) => ({ ...x, after: null })),
    ),
    selectedId: "dm_rate_horizon_plus_200bps",
  });
  ...
  expect(`${noAfter.headline.dek} ${noBefore.headline.dek}`).not.toMatch(
    /shock/,
  );
});
...
test("rows beside a withheld Cash book are a cannot-say, never a negative", () => {
  const readdressed = {
    ...
    scenarios: DEMO_STRESS_NEAR.scenarios.map((s) => ({
      ...s,
      results: s.results.map((r) => ({ ...r, account: UNKNOWABLE_ADDR })),
    })),
  };
  const w = addressWorkspace({
    address: UNKNOWABLE_ADDR,
    view: view({
      address: UNKNOWABLE_ADDR,
      lookup: { phase: "ready", value: lookup(ADDRESS_UNKNOWABLE) },
      stress: { phase: "ready", value: lookup(readdressed) },
    }),
    selectedId: "eth_minus_30",
  });
  expect(w.state).toBe("rows");
  ...
  expect(`${w.headline.emphasis} ${w.headline.dek}`).not.toContain(
    "No Cash position",
  );
});
...
test("rows beside no readable position: an unreadable scale and a missing Cash position each say so, never the scenarios' sentence", () => {
  const unreadable = {
    ...DEMO_ADDRESS_NEAR,
    positions: DEMO_ADDRESS_NEAR.positions.map((p) => ({
      ...p,
      value_decimals: 1.5,
    })),
  };
  const scale = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: view({
      lookup: { phase: "ready", value: lookup(unreadable) },
      stress: { phase: "ready", value: lookup(DEMO_STRESS_NEAR) },
    }),
    selectedId: "eth_minus_30",
  });
  ...
  expect(scale.headline).toEqual({
    emphasis: "The Cash position's scale could not be read.",
    rest: "",
    tone: "refused",
    dek: "No figure prints at an unreadable scale.",
  });
  ...
  const missing = addressWorkspace({
    ...
    view: view({
      ...
      stress: {
        phase: "ready",
        value: lookup({ ...DEMO_STRESS_NEAR, address: NOT_FOUND_ADDR }),
      },
    }),
    selectedId: null,
  });
...
test("rowOutcome: the library word is the row's own verdict — flips, cannot-say, the engine's reason, or not on this address", () => {
  const rows = addressWorkspace({
    address: DEMO_NEAR_ADDR,
    view: nearWith(DEMO_STRESS_NEAR),
    selectedId: "eth_minus_30",
  }).rows;
  const eth = rows.find((r) => r.id === "eth_minus_30");
  expect(eth?.flips).toBe(true);
  expect(rowOutcome(eth)).toEqual({
    key: "result",
    text: "Becomes liquidatable",
    tone: "crit",
  });
  expect(rowOutcome({ ...eth!, flips: false })).toEqual({
    key: "result",
    text: "Stays inside its cap",
    tone: "ok",
  });
  expect(rowOutcome({ ...eth!, flips: null })).toEqual({
    key: "withheld",
    text: "Cannot say",
    tone: "refused",
  });
  expect(
    rowOutcome({ ...eth!, applicable: false, reason: "no Cash position" }),
  ).toEqual({
    key: "not-covered",
    text: "Not applicable: no Cash position",
    tone: "dim",
  });
  expect(rowOutcome({ ...eth!, applicable: false, reason: null })).toEqual({
    key: "not-covered",
    text: "Not applicable: the engine gave no reason",
    tone: "dim",
  });
  expect(rowOutcome(undefined)).toEqual({
    key: "not-covered",
    text: "Not on this address",
    tone: "dim",
  });
});
