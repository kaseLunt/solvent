...
- **Task 10 — Anvil-fork replay test + phase gate** *(after 9)*: opt-in integration test (`ANVIL_FORK_RPC` env gates it; skipped in CI, documented honestly) — anvil-fork OP at a hash-bound pinned block, assert derived borrower state (read as-of the pin from the live DB) equals direct fork view calls, census-gated with a pinned borrower/stratum fixture [AMENDED 2026-07-27, owner-delegated P2-exit decision C2: the original walker+runner-against-the-fork phrasing is superseded — the delivered derived-vs-fork comparison proves output correctness at the pin, and the ingestion mechanism was exercised by Task 9's from-scratch full-history backfill; a hermetic walker/runner-vs-fork pipeline-replay harness is deferred to the P3 backlog as a named item]; then W1 closure: evidence receipts per the control plane (`doctor.py --receipt-basis W1 --snapshot <commit>` → receipt file → `doctor.py --stamp W1` → status `achieved`), whole-branch review (standard + Codex per standing directive), CI green, phase report.
...
Opt-in `ANVIL_FORK_RPC` integration test asserting derived borrower state against direct fork view calls at a hash-bound pinned OP block, census-gated with a pinned borrower/stratum fixture [AMENDED 2026-07-27 per the §Tasks 3b–10 Task-10 entry — owner-delegated P2-exit decision C2; pipeline-replay harness deferred to the P3 backlog]; then W1 closure — receipts + `doctor.py --stamp W1`, whole-branch review (standard + Codex per D-005), CI green, phase report, STATUS/ROADMAP transition.
