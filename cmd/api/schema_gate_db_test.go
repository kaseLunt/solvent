...
func TestSchemaGateRefusesAMismatchedDatabase(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(14), want,
		"this build's queries are written against migration 14; if the migration set moved, this expectation and the operator runbook both need updating deliberately")
